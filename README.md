...
* **fast and easy roll out of services** (on different systems, is possible - roll out with Docker Compose)
* **completeness** (all logs are completely collected and displayed)
* **query language is user-friendly** (and easy to use)
* **intuitive** (the visualization tools Grafana and Kibana are intuitive and easy to use)
* **well documented** (detailed and comprehensible documentation is available)
...
to collectors, which in turn forwards them to a central location.
...
and known as ELK or EFK Stack, while the abbrevitation stands for **E**lasticsearch (object store), **L**ogstash or
...
The PLG stack, which consists of **P**romtail, **L**oki, and **G**rafana, is mostly known as Grafana Loki. The Promtail
agent is designed for Loki installations, but many other agents integrate seamlessly with Loki. For example, Fluentd and
Logstash.
...
``` js
{ app="nginx" } |= "GET"          // match exactly 'GET'
{ app="nginx" } |~ "200|201|202"  // regex match 'either 200, 201, or 202'
{ app="nginx" } != "GET"          // do not match 'GET'
{ app="nginx" } !~ "200|201|202"  // do not match 'either 200, 201 or 202'
```

#### Labels

Labels are key-value pairs and can be defined arbitrarily, including in the Scrape Configuration. The combination of key
and value of each label defines a stream (see this [grafana blog post][15]).
...
logfmt is a parser and can be added using the `| logfmt` pipe and will extract all keys and values from the logfmt
formatted log line. Keys from detected fields can then be used like labels. An example query which gives the sum of all
http status codes of the last minute could look like this:
...
(which, if needed, can be optionally extended). These labels can then be used to execute metric or filter queries.
...
There are three different ways to roll out Loki.
...
1. Create a docker container for Loki, Promtail and Grafana using Docker compose
0. Create a config file for Promtail
0. Setup Loki as data source in Grafana
0. Analyze the data that available in the Loki data source

#### Docker-Compose File for PLG

The file [docker-compose.yml](loki-promtail-example/docker-compose.yml) defines the individual services (Loki, Promtail
...
In the folder `app-promtail` you will find another [Docker-compose](loki-promtail-example/app/nginx-example.yaml) file.
...
The three tools Promtail, Grafana and Loki are open source and can be used free of charge. The configuration of the
technology stack is particularly easy, requiring only a promtail configuration. However, the yaml format is not
...
1. Create docker container for Loki, Fluentd and Grafana using Docker compose
0. Create a config file for Fluentd
0. Setup Loki as data source in Grafana
0. Analyze the data that available in the Loki data source
...
* `label`  groups the output and filters for internal routing
...
previously described configuration. See section
[Setup Loki as data source in Grafana](#setup-loki-as-data-source-in-grafana).
...
the table were made on a scale of 1 to 5, with 5 being the best rating.
...
1. Prepare docker container for Elasticsearch, Fluentd and Kibana using Docker compose
0. Create a Fleuntd config and Dockerfile
0. Analyze the data that is available
...
Once you have everything prepared, you can start the services. To do this, navigate to the folder
[efk-example](efk-example/) and execute the following Docker commands.
...
| Completeness                   | 5            |
| Query language is user-friendly| 5            |
| Intuitive visualization tool   | 5            |
| Documentation                  | 3            |

When it comes to collecting and analyzing data, the technology stack of Elasticsearch, Kibana and Fluentd is a popular
choice. Elasticsearch, which is built on the open source Apache Lucene search engine library, is free to try, except for
some security features, among others, which are chargeable. Kibana and Fluentd are both open source and free to use.

The technology stack is relatively easy to configure, requiring only a fluentd configuration. However, it should be
noted that the configuration is in config format and the concepts of fluentd must be understood in order to work
successfully with the tool. The large selection of plugins is on the one hand an advantage, but can certainly lead to
excessive demands, since some of the plugins are poorly documented.

Rolling out the Elasticsearch technology stack via Docker Compose is possible without any problems. However, since it is
very resource hungry, it takes significantly longer to initialize than, for example, the Loki stack. The logs generated
using the ApacheBench tool could be fully displayed in Kibana. Creating a dashboard in Kibana is intuitive, and created
dashboards can be easily exported and imported via an API interface. If one wants to search the logs directly in
Elasticsearch using the query language - Query DSL this is intuitive and well documented.The logs generated using the
ApacheBench tool could be fully displayed in Kibana. Creating a dashboard in Kibana is intuitive, and created dashboards
can be easily exported and imported via an API interface. If one wants to search the logs directly in Elasticsearch
using the query language - Query DSL this is intuitive and well documented.

Overall, the documentation for all tools is very good. As already mentioned, only the documentation of the plugins
leaves much to be desired. However, there is a large community that helps with questions and offers tutorials, e.g. in
the form of YouTube videos, free of charge.

# Conclusion

In summary, all three technology stacks are free (at least in their base configuration), easy to configure, and can be
deployed quickly and easily with Docker Compose.
...
The two query languages Elasticsearch Query DSL and Grafana's LogQL are relatively easy to understand and use. However
they have differences in available features and of course syntax.
...
a bit difficult to find the right information, especially if you're not that familiar with the subject's matter.

To finaly conclude, all three technology stacks are very good options for logging applications and systems. They offer a
variety of features and configuration options to effectively collect, store and analyze data. Every has an active
community that can answer questions and solve problems. So, the choice of the right technology stack depends on specific
requirements and preferences.

# Outlook

Working with technologies such as Promtail, Loki, Grafana, Elasticsearch, Fluentd, and Kibana provides an opportunity to
